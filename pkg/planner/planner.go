...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/rancher/cluster-api-provider-rancher/pkg/infofunctions"
	"hash/crc32"
	"math"
	"path"
	"path/filepath"
...
type InfoFunctions struct {
	infofunctions.ImageResolver
	infofunctions.ReleaseData
	infofunctions.SystemAgentImage
	infofunctions.SystemPodLabelSelectors
	infofunctions.ControlPlaneManifests
}

func New(cContext *caprcontext.Context, functions InfoFunctions) *Planner {
	store := NewStore(cContext.Core.Secret(),
		cContext.CAPI.Machine().Cache())
	return &Planner{
		ctx:               cContext.Ctx,
		store:             store,
		machines:          cContext.CAPI.Machine(),
		machinesCache:     cContext.CAPI.Machine().Cache(),
		secretClient:      cContext.Core.Secret(),
		secretCache:       cContext.Core.Secret().Cache(),
		configMapCache:    cContext.Core.ConfigMap().Cache(),
		capiClient:        cContext.CAPI.Cluster(),
		capiClusters:      cContext.CAPI.Cluster().Cache(),
		rkeControlPlanes:  cContext.RKE.RKEControlPlane(),
		rkeBootstrap:      cContext.RKE.RKEBootstrap(),
		rkeBootstrapCache: cContext.RKE.RKEBootstrap().Cache(),
		etcdSnapshotCache: cContext.RKE.ETCDSnapshot().Cache(),
		etcdS3Args: s3Args{
			secretCache: cContext.Core.Secret().Cache(),
		},
...
func (p *Planner) desiredPlan(controlPlane *rkev1.RKEControlPlane, tokensSecret plan.Secret, entry *planEntry, joinServer string) (plan.NodePlan, string, error) {
	...
	if isInitNode(entry) {
		nodePlan, err = p.addKubeconfigDumpPeriodicInstruction(nodePlan)
		if err != nil {
			return nodePlan, joinedTo, err
		}
	}
