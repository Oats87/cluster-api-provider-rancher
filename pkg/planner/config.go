...
// TODO: from: "github.com/rancher/rancher/pkg/controllers/management/secretmigrator"
const (
...
func addRoleConfig(config map[string]interface{}, controlPlane *rkev1.RKEControlPlane, entry *planEntry, joinServer string) string {
	...
	if pr := capr.GetPrivateRepoURLFromControlPlane(controlPlane); !isOnlyWorker(entry) {
		// Fallback to the system default registry if the private repo URL is not defined at a cluster level
		if pr == "" {
			pr = caprsettings.SystemDefaultRegistry.Get()
		}
		if pr != "" {
...
func retrieveClusterAuthorizedSecret(secret *v1.Secret, clusterName string) ([]byte, error) {
	authorized, ownerFound := clusterObjectAuthorized(secret, AuthorizedSecretAnnotation, clusterName)
