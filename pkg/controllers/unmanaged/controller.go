...
func Register(wContext *caprcontext.Context, kcManager kubeconfig.Manager) {
	h := handler{
		unmanagedMachine: wContext.RKE.CustomMachine(),
		rkeClusterCache:  wContext.RKE.RKECluster().Cache(),
		//mgmtClusterCache:  clients.Mgmt.Cluster().Cache(),
		//clusterCache:      clients.Provisioning.Cluster().Cache(),
		capiClusterCache: wContext.CAPI.Cluster().Cache(),
		machineCache:     wContext.CAPI.Machine().Cache(),
		machineClient:    wContext.CAPI.Machine(),
		secrets:          wContext.Core.Secret(),
		apply: wContext.Apply.WithSetID("unmanaged-machine").
			WithCacheTypes(
				//clients.Mgmt.Cluster(),
				//clients.Provisioning.Cluster(),
				wContext.RKE.CustomMachine(),
				wContext.CAPI.Machine(),
				wContext.RKE.RKEBootstrap()),
	}
	wContext.RKE.CustomMachine().OnRemove(wContext.Ctx, "unmanaged-machine", h.onUnmanagedMachineOnRemove)
	wContext.RKE.CustomMachine().OnChange(wContext.Ctx, "unmanaged-health", h.onUnmanagedMachineChange)
	wContext.Core.Secret().OnChange(wContext.Ctx, "unmanaged-machine-secret", h.onSecretChange)

	relatedresource.Watch(wContext.Ctx, "unmanaged-machine", func(namespace, name string, obj runtime.Object) ([]relatedresource.Key, error) {
		if rkeCluster, ok := obj.(*rkev1.RKECluster); ok {
			...
			if rkeCluster.Annotations[capr.DeleteMissingCustomMachinesAfterAnnotation] != "" {
				...
				machines, err := wContext.CAPI.Machine().List(rkeCluster.Namespace, metav1.ListOptions{LabelSelector: capi.ClusterNameLabel + "=" + rkeCluster.Annotations[capi.ClusterNameLabel]})
				if err != nil {
					return nil, err
				}
			...
			if m.Spec.InfrastructureRef.Kind == UnmanagedMachineKind && m.Spec.InfrastructureRef.APIVersion == capr.RKEAPIVersion {
				...
				rkeCluster, err := wContext.RKE.RKECluster().Get(m.Namespace, m.Spec.ClusterName, metav1.GetOptions{})
				if err != nil {
					return nil, err
				}
		...
		return nil, nil
	}, wContext.RKE.CustomMachine(), wContext.RKE.RKECluster(), wContext.CAPI.Machine())
}
...
func (h *handler) getCAPICluster(secret *corev1.Secret) (*capi.Cluster, error) {
	...
	rClusters, err := h.clusterCache.GetByIndex(ClusterV1ByClusterV3Reference, cluster.Name)
	...
	if rkeCluster.Annotations[capr.DeleteMissingCustomMachinesAfterAnnotation] != "" {
		...
		if machineHasNodeNotFoundCondition(capiMachine) {
			...
			if now.After(lastTransition.Time.Add(d)) {
				restConfig, err := h.kubeconfigManager.GetRESTConfig(capiMachine.Namespace, capiMachine.Spec.ClusterName)
				if err != nil {
					return customMachine, err
				}
