...
import (
	...
	v1 "github.com/rancher/cluster-api-provider-rancher/pkg/apis/rke.cattle.io/v1"
...
func Register(wContext *caprcontext.Context) {
	h := handler{
		rkeCluster:       wContext.RKE.RKECluster(),
		capiClusterCache: wContext.CAPI.Cluster().Cache(),
	}

	wContext.RKE.RKECluster().OnChange(wContext.Ctx, "rke-cluster", h.OnChange)
	relatedresource.Watch(wContext.Ctx, "rke-cluster-trigger", func(namespace, name string, obj runtime.Object) ([]relatedresource.Key, error) {
		...
		return nil, nil
	}, wContext.RKE.RKECluster(), wContext.CAPI.Cluster())
}
