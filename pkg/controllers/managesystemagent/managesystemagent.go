...
import (
	...
	v1 "github.com/rancher/cluster-api-provider-rancher/pkg/generated/controllers/rke.cattle.io/v1"
...
type InfoFunctions struct {
	infofunctions.GetToken
}

func Register(wContext *caprcontext.Context, i InfoFunctions) {
	h := &handler{
		functions: i,
	...
	v1.RegisterRKEControlPlaneStatusHandler(wContext.Ctx, clients.RKE.RKEControlPlane(),
		"", "monitor-system-upgrade-controller-readiness", h.syncSystemUpgradeControllerStatus)

	rocontrollers.RegisterClusterGeneratingHandler(wContext.Ctx, clients.Provisioning.Cluster(),
		clients.Apply.
...
func installer(cluster *rancherv1.Cluster, secretName string) []runtime.Object {
	...
	if cluster.Spec.RKEConfig.DataDirectories.SystemAgent != "" {
		env = append(env, corev1.EnvVar{
			...
			Value: capr.GetSystemAgentDataDir(&rkev1.RKEClusterSpecCommon{
				DataDirectories: rkev1.DataDirectories{
					SystemAgent: cluster.Spec.RKEConfig.RKEClusterSpecCommon.DataDirectories.SystemAgent,
				},
			}),
		})
	}
