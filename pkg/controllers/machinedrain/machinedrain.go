...
func Register(ctx context.Context, cContext *caprcontext.Context) {
	h := &handler{
		ctx:          ctx,
		machineCache: cContext.CAPI.Machine().Cache(),
		secrets:      cContext.Core.Secret(),
		secretCache:  cContext.Core.Secret().Cache(),
	}

	cContext.Core.Secret().OnChange(ctx, "machine-drain", h.OnChange)
}
