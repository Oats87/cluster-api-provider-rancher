...
func Register(wContext *caprcontext.Context, planner *caprplanner.Planner) {
	h := handler{
		planner:       planner,
		controlPlanes: wContext.RKE.RKEControlPlane(),
	}
	v1.RegisterRKEControlPlaneStatusHandler(wContext.Ctx, wContext.RKE.RKEControlPlane(), "", "planner", h.OnChange)
	relatedresource.Watch(wContext.Ctx, "planner", func(namespace, name string, obj runtime.Object) ([]relatedresource.Key, error) {
		...
		return nil, nil
	}, wContext.RKE.RKEControlPlane(), wContext.Core.Secret(), wContext.CAPI.Machine(), wContext.Core.ConfigMap())
}
