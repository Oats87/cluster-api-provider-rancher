...
import (
	...
	"github.com/rancher/cluster-api-provider-rancher/pkg/standalone/kubeconfig"
	...
	"github.com/rancher/cluster-api-provider-rancher/pkg/controllers/etcdmgmt"
...
type handler struct {
	...
	rkeControlPlanes    rkecontrollers.RKEControlPlaneCache
	rkeBootstrap        rkecontrollers.RKEBootstrapController
...
func Register(wContext *caprcontext.Context) {
	h := &handler{
		...
		rkeControlPlanes:    wContext.RKE.RKEControlPlane().Cache(),
		rkeBootstrap:        wContext.RKE.RKEBootstrap(),
		k8s:                 wContext.K8s,
	}

	wContext.RKE.RKEBootstrap().OnChange(wContext.Ctx, "rke-bootstrap-cluster-name", h.OnChange)
	wContext.RKE.RKEBootstrap().OnRemove(wContext.Ctx, "rke-bootstrap-etcd-removal", h.OnRemove)
	rkecontrollers.RegisterRKEBootstrapGeneratingHandler(wContext.Ctx,
		wContext.RKE.RKEBootstrap(),
		wContext.Apply.
			WithCacheTypes(
				wContext.RBAC.Role(),
				wContext.RBAC.RoleBinding(),
				wContext.CAPI.Machine(),
				wContext.Core.ServiceAccount(),
	...
	relatedresource.Watch(wContext.Ctx, "rke-bootstrap-trigger", func(namespace, name string, obj runtime.Object) ([]relatedresource.Key, error) {
		...
		return nil, nil
	}, wContext.RKE.RKEBootstrap(), wContext.Core.ServiceAccount(), wContext.CAPI.Machine())
}
...
func (h *handler) getBootstrapSecret(namespace, name string, envVars []corev1.EnvVar, machine *capi.Machine, dataDir string) (*corev1.Secret, error) {
	...
	if machine.Spec.Version != nil && *machine.Spec.Version != "" {
		...
		data, err = is(base64.URLEncoding.EncodeToString(hash[:]), envVars, serverURL, ca, dataDir)
		if err != nil {
			return nil, err
		}
	}
...
func (h *handler) reconcileMachinePreTerminateAnnotation(bootstrap *rkev1.RKEBootstrap) (*rkev1.RKEBootstrap, error) {
	...
	kcSecret, err := h.secretCache.Get(bootstrap.Namespace, kubeconfig.GetKubeConfigSecretName(bootstrap.Spec.ClusterName))
	if err != nil {
		if apierrors.IsNotFound(err) {
			return h.ensureMachinePreTerminateAnnotationRemoved(bootstrap, machine)
		}
