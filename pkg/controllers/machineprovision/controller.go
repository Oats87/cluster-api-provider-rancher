...
func Register(wContext *caprcontext.Context) {
	h := &handler{
		ctx: wContext.Ctx,
		apply: wContext.Apply.WithCacheTypes(wContext.Core.Secret(),
			wContext.Core.ServiceAccount(),
			wContext.RBAC.RoleBinding(),
			wContext.RBAC.Role(),
			//wContext.Batch.Job()
		),
		pods: wContext.Core.Pod().Cache(),
		//jobController:    wContext.Batch.Job(),
		//jobs: wContext.Batch.Job().Cache(),
		secrets:          wContext.Core.Secret().Cache(),
		machineCache:     wContext.CAPI.Machine().Cache(),
		machineClient:    wContext.CAPI.Machine(),
		machineSetCache:  wContext.CAPI.MachineSet().Cache(),
		capiClusterCache: wContext.CAPI.Cluster().Cache(),
		//nodeDriverCache:     wContext.Mgmt.NodeDriver().Cache(),
		namespaces: wContext.Core.Namespace().Cache(),
		dynamic:    wContext.Dynamic,
		//rancherClusterCache: wContext.Provisioning.Cluster().Cache(),
		//kubeconfigManager: kubeconfigManager,
	}

	removeHandler := generic.NewRemoveHandler("machine-provision-remove", wContext.Dynamic.Update, h.OnRemove)

	wContext.Dynamic.OnChange(wContext.Ctx, "machine-provision-remove", validGVK, dynamic.FromKeyHandler(removeHandler))
	wContext.Dynamic.OnChange(wContext.Ctx, "machine-provision", validGVK, h.OnChange)
	//wContext.Batch.Job().OnChange(wContext.Ctx, "machine-provision-pod", h.OnJobChange)
}
...
// TODO: from nodedriver
var SSHKeyFields = map[string]bool{
	"sshKeyContents": true,
	"sshKey":         true,
	"privateKeyFile": true,
}
	...
	if fields, ok := node.SchemaToDriverFields[driver]; ok {
		for schemaField, driverField := range fields {
			if fileContents, ok := config[schemaField].(string); ok {
				...
				if ok := SSHKeyFields[schemaField]; ok {
					fileName = "id_rsa"
				}
